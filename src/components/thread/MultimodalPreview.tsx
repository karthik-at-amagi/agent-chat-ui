...
export const MultimodalPreview: React.FC<MultimodalPreviewProps> = ({
  ...
  if (block.type === "file" && block.mimeType === "application/pdf") {
    ...
    return (
      <div
        className={cn(
          "border-border bg-muted relative flex items-start gap-2 rounded-md border px-3 py-2",
          className,
        )}
      >
        ...
        {removable && (
          <button
            type="button"
            className="bg-accent text-accent-foreground hover:bg-accent/80 ml-2 self-start rounded-full p-1"
            onClick={onRemove}
  ...
  return (
    <div
      className={cn(
        "border-border bg-muted text-muted-foreground flex items-center gap-2 rounded-md border px-3 py-2",
        className,
      )}
    >
      ...
      {removable && (
        <button
          type="button"
          className="bg-accent text-accent-foreground hover:bg-accent/80 ml-2 rounded-full p-1"
          onClick={onRemove}
